...
function renderMarkdownReport(report) {
    ...
    for (const item of report.evidence) {
        lines.push(`| ${escapeTable(item.name)} | ${STATUS_ICONS[item.status]} ${item.status} | ` +
    ...
    if (report.scoreBreakdown.deductions.length > 0) {
        ...
        for (const deduction of report.scoreBreakdown.deductions) {
            lines.push(`- **−${String(deduction.points)}** ${escapeInline(deduction.reason)} ` +
    ...
    if (report.repository.changedFiles.length === 0) {
        ...
        if (report.repository.changedFiles.length > 100) {
            lines.push(`- …and ${String(report.repository.changedFiles.length - 100)} more`);
        }
    }
    const details = report.evidence.filter((item) => item.details !== undefined && item.details.length > 0);
    if (details.length > 0) {
        lines.push("", "## Command output summaries", "");
        for (const item of details) {
            lines.push(`<details><summary>${escapeHtml(item.name)}</summary>`, "", `<pre>${escapeHtml(item.details ?? "")}</pre>`, "", "</details>", "");
        }
    }
    lines.push("", "_Deterministic checks first. Semantic judgment second. Human decision last._", "");
    return lines.join("\n");
}
